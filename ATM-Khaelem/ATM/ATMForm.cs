    ...
    public partial class ATMForm : Form
    {
        ...
        private async Task dispWithdraw()
        {
            ...
            if (input > 0 && input < 4)
            {
                ...
                if (input == 1)
                {
                    //if mode is lock(mode==2)Wait to get access from lock to ensure no race conditions happen
                    if (mode == 2)
                        Global._access.WaitOne();

                    //attempt to decrement account by 10 punds
                    if (activeAccount.decrementBalLock(10))
                    {
                        //if this is possible display new balance and await key press
                        write("new balance " + activeAccount.getBalance());
                        write("(prese enter to continue)");
                ...
                else if (input == 2)
                {
                    ...
                    if (mode == 2)
                        Global._access.WaitOne();
                    if (activeAccount.decrementBalLock(50))
                    {
                        write("new balance " + activeAccount.getBalance());
                        write(" (prese enter to continue)");
                ...
                else if (input == 3)
                {
                    ...
                    if (mode == 2)
                        Global._access.WaitOne();
                    if (activeAccount.decrementBalLock(500))
                    {
                        write("new balance " + activeAccount.getBalance());
                        write(" (prese enter to continue)");
